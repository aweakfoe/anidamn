...
function handleWinLoss(won) {
    ...
    if (won) {
        ...
        gameOverTitle.className = "font-display-xl text-4xl mb-4 text-match-success";
        ...
        gameOverTitle.className = "font-display-xl text-4xl mb-4 text-error";
...
function createBox(label, value, compResult, delayIdx, isRestoring) {
    let frontClass = "glass-card border border-white/5 shadow-lg";
    let backClass = "glass-card border border-white/10 shadow-lg";
    ...
    let overlayClass = compResult.status === 'match' ? 'bg-match-success/20' :
                       compResult.status === 'partial' ? 'bg-yellow-500/20' :
                       'bg-surface-variant/30';
    
    let borderClass = compResult.status === 'match' ? 'border-match-success/50' :
                      compResult.status === 'partial' ? 'border-yellow-500/50' :
                      'border-white/5';
                       
    let textColor = compResult.status === 'match' ? 'text-match-success' : 
                    compResult.status === 'partial' ? 'text-yellow-400' : 'text-on-surface';
    ...
    <div class="attr-container rounded-xl">
        <div class="attribute-box-inner ${isRestoring ? 'flip' : 'animate-flip'}" style="${!isRestoring ? `animation-delay: ${delay};` : ''}">
            <div class="attribute-front rounded-xl ${frontClass} bg-surface-variant/30 flex items-center justify-center">
                <span class="material-symbols-outlined text-outline text-3xl opacity-50">help</span>
            </div>
            <div class="attribute-back rounded-xl ${backClass} ${overlayClass} ${borderClass} flex flex-col items-center justify-center">
                <span class="font-label-sm text-xs ${labelColor} uppercase tracking-wider mb-1">${label}</span>
                <div class="flex items-center gap-1 font-body-md font-semibold ${textColor}">
                    ${value} ${arrowHtml}
                </div>
            </div>
        </div>
...
function makeGuess(anime, isRestoring = false) {
    ...
    card.className = "glass-card rounded-2xl p-6 md:p-8 flex flex-col gap-6 w-full relative overflow-hidden border border-white/10 shadow-2xl transition-all hover:bg-white/5";
